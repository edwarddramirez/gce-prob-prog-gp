...
class EbinPoissonModel:
    ...
    u_grid_type : {'healpix_bins', 'healpix'}
        Type of grid for inducing points.
    u_weights : None or array
        Weights for inducing points.
    Np : int
    ...
    def __init__(
        ...
        # v0.2
        svi_seed = 42, # TODO: NEEDS TO BE ADDED TO POISSON MODEL OF INPUT CELL
    ):
    ...
    def config_model(self, ebin=10):
        ...
        if 'nfw' in self.temp_list:
            ...
            if self.nfw_gamma != 'vary':
                ...
                self.nfw_temp_fixed = self.nfw_temp.get_NFW2_masked_template(gamma=gamma)
        ...
        if self.is_gp:
            ...
            if self.u_option == 'float':
                ...
                # made using ../scratch/grid.ipynb in gce-prob-prog-ed-v0.2 main directory
                    ...
                    self.xu_f = 20. * ef.sunflower(Nu, 0) # multiply by radius to extend to 20 degree circle
...
# ====== Numpyro Implementation of Model ======
    def model(self, ebin=10):
        ...
        if 'iso' in self.temp_list:
            S_iso = numpyro.sample('S_iso', dist.Uniform(1e-3, 5000))
        if 'psc' in self.temp_list:
            S_psc = numpyro.sample('S_psc', dist.Uniform(1e-3, 5000))
        if 'bub' in self.temp_list:
            S_bub = numpyro.sample('S_bub', dist.Uniform(1e-3, 5000))
        
        if self.n_dif_temps > 0:
            if 'pib' in self.temp_list:
                S_pib = numpyro.sample('S_pib', dist.Uniform(1e-3, 10000))
            if 'ics' in self.temp_list:
                S_ics = numpyro.sample('S_ics', dist.Uniform(1e-3, 10000))
        ...
        if 'blg' in self.temp_list:
            if self.n_blg_temps == 1:
                S_blg = numpyro.sample('S_blg', dist.Uniform(1e-3, 1000))
            else:
...
#                 gamma = self.nfw_gamma
        ...
        if 'dsk' in self.temp_list:
            if self.disk_option in ['vary', 'fixed']:
                S_dsk = numpyro.sample('S_dsk', dist.Uniform(1e-3, 500))
        ...
        with numpyro.plate('data', size=len(data), dim=-1, subsample_size=self.Nsub) as ind:
            ...
            if 'nfw' in self.temp_list:
                if self.nfw_gamma == 'vary':
                    mu += S_nfw * self.nfw_temp.get_NFW2_masked_template(gamma=gamma)[ind]
                else:
            ...
            if self.is_gp:
                ...
                sample_keys = jax.random.split(jax.random.PRNGKey(self.svi_seed), 3)
    ...
    def fit_SVI(
        ...
        **model_static_kwargs,
        ...
        # TODO: Need to modify INPUT CELLS and exported modules to include this information;
        # that would be all you have to change. You will define the optimizer in the 
